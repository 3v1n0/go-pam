...
func Test_Status(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("can't create service path %v", err)
	}
	...
	for r, expected := range retTypes {
		for actionName, action := range actions {
			t.Run(fmt.Sprintf("%s %s", r, actionName), func(t *testing.T) {
				serviceFile := filepath.Join(servicePath, r)
				...
				if err := os.WriteFile(serviceFile+"-"+actionName,
					[]byte(contents), 0600); err != nil {
					t.Fatalf("can't create service %v file %v", r, err)
				}

				tx, err := StartConfDir(r+"-"+actionName, "", c, servicePath)
				if err != nil {
					t.Fatalf("start #error: %v", err)
				}

...
func (m *moduleTransaction) startConvImpl(iface moduleTransactionIface, req ConvRequest) (
	ConvResponse, error) {
	if resp, err := m.startConvMultiImpl(iface, []ConvRequest{req}); err != nil {
		return nil, err
	} else if len(resp) != 1 {
		return nil, NewTransactionError(ErrConv, errors.New("not enough values returned"))
	} else {
		return resp[0], nil
	}
}
...
func (m *moduleTransaction) startConvMultiImpl(iface moduleTransactionIface,
	...
	if len(requests) == 0 {
		return nil, NewTransactionError(ErrConv, errors.New("no requests defined"))
	}
	if len(requests) > maxNumMsg {
		return nil, NewTransactionError(ErrConv, errors.New("too many requests"))
	}
	...
	if conv == nil || conv.conv == nil {
		status := ErrConv
		_ = m.handlePamStatus(C.int(ErrConv))
		return nil, NewTransactionError(status,
			errors.New("impossible to find conv handler"))
	}
	...
	for i, req := range requests {
		...
		case BinaryConvRequester:
			if !iface.hasBinaryProtocol() {
				return nil, NewTransactionError(ErrConv, fmt.Errorf(
		...
		default:
			return nil, NewTransactionError(ErrConv,
				fmt.Errorf(
					"unsupported conversation type %#v", r))
		}
	...
	for i, resp := range responses {
		...
		case BinaryPrompt:
			...
			goReplies = append(goReplies, resp)
		default:
			status := ErrConv
			_ = m.handlePamStatus(C.int(ErrConv))
			return nil, NewTransactionError(status,

...
func Test_ModuleTransaction_InvokeHandler(t *testing.T) {
	...
	transactionError.lastStatus.Store(int32(ErrSystem))

	moduleTransactionError := &moduleTransaction{}
	moduleTransactionError.lastStatus.Store(int32(ErrSystem))
	...
	tests := map[string]struct {
		...
		"ignore": {
			...
			returnedError: NewTransactionError(ErrIgnore, nil),
		},
		...
		"transaction-error-service-error": {
			...
			returnedError:    NewTransactionError(ErrService, errors.New("ErrService")),
		...
		t.Run(name, func(t *testing.T) {
			...
			if err != nil {
				...
				if errors.As(err, &txErr) {
					expectedStatus = txErr.Status()
				} else {
					expectedStatus = ErrAbort
				}
			}

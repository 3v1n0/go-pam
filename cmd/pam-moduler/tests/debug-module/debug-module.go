//go:generate go run github.com/msteinert/pam/cmd/pam-moduler -libname "pam_godebug.so"
...
func (dm *DebugModule) handleCall(args []string, action string) error {
	...
	var status pam.Error
	if !errors.As(err, &status) {
		panic(fmt.Sprintf("Unexpected error %v\n", err))
	}

	return pam.NewTransactionError(status, fmt.Errorf("Error %v", err))
}
